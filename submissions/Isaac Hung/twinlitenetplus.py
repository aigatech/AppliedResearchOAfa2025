...
# Source: https://github.com/chequanghuy/TwinLiteNetPlus (with minor modifications)
...
def show_seg_result(img, da_seg_mask, ll_seg_mask):
    ...
    # Green for drivable area, Red for lane lines
    color_area[da_seg_mask == 1] = [0, 255, 0]  # Green for drivable area
    color_area[ll_seg_mask == 1] = [255, 0, 0]  # Red for lane lines
    ...
    result_img = img.copy()
    result_img[color_mask != 0] = (
        result_img[color_mask != 0] * 0.5 + color_seg[color_mask != 0] * 0.5
    )
...
class Encoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.b1 = ConvBatchnormRelu(
            model_cfg["chanels"][0] + chanel_img, model_cfg["chanels"][1]
        )
        ...
        for i in range(0, model_cfg["p"]):
            self.level2.append(
                DepthwiseESP(model_cfg["chanels"][2], model_cfg["chanels"][2])
            )
        self.b2 = ConvBatchnormRelu(
            model_cfg["chanels"][3] + chanel_img, model_cfg["chanels"][3] + chanel_img
        )

        self.level3_0 = StrideESP(
            model_cfg["chanels"][3] + chanel_img, model_cfg["chanels"][3]
        )
        self.level3 = nn.ModuleList()
        for i in range(0, model_cfg["q"]):
            self.level3.append(
                DepthwiseESP(model_cfg["chanels"][3], model_cfg["chanels"][3])
            )
...
class SingleLiteNetPlus(nn.Module):
    ...
    def __init__(self, args=None):
        super().__init__()
        self.encoder = Encoder("nano")

        self.caam = CAAM(
            feat_in=sc_ch_dict["nano"]["chanels"][2],
            num_classes=sc_ch_dict["nano"]["chanels"][2],
            bin_size=(2, 4),
            norm_layer=nn.BatchNorm2d,
        )
        self.conv_caam = ConvBatchnormRelu(
            sc_ch_dict["nano"]["chanels"][2],
            sc_ch_dict["nano"]["chanels"][1],
        )

        self.up_1 = UpConvBlock(
            sc_ch_dict["nano"]["chanels"][1],
            sc_ch_dict["nano"]["chanels"][0],
        )  # out: Hx4, Wx4
        self.up_2 = UpConvBlock(sc_ch_dict["nano"]["chanels"][0], 8)  # out: Hx2, Wx2
...
class TwinLiteNetPlus(nn.Module, PyTorchModelHubMixin):
    """
    This class defines the ESPNet network
    """
    ...
    def __init__(self):
        ...
        self.up_1_da = UpConvBlock(
            sc_ch_dict["nano"]["chanels"][1],
            sc_ch_dict["nano"]["chanels"][0],
        )  # out: Hx4, Wx4
        self.up_2_da = UpConvBlock(sc_ch_dict["nano"]["chanels"][0], 8)  # out: Hx2, Wx2
        self.out_da = UpConvBlock(8, 2, last=True)

        self.up_1_ll = UpConvBlock(
            sc_ch_dict["nano"]["chanels"][1],
            sc_ch_dict["nano"]["chanels"][0],
        )  # out: Hx4, Wx4
        self.up_2_ll = UpConvBlock(sc_ch_dict["nano"]["chanels"][0], 8)  # out: Hx2, Wx2
